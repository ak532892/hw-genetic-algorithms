function population = crossover(pool, population)
    ...
    global POPULATION_CNT CROSSOVER_RATE UNKNOWN_CNT
    ...
    while(cnt < POPULATION_CNT)
        ...
        if (crossover_if > CROSSOVER_RATE)
            ...
            for i = 1:pos
                ...
                population(cnt).genes(pos+1:UNKNOWN_CNT) = pool(pos2).genes(pos+1:UNKNOWN_CNT);
                population(cnt+1).genes(pos+1:UNKNOWN_CNT) = pool(pos1).genes(pos+1:UNKNOWN_CNT);
            end
        end
        cnt = cnt+2; % 已複製完二條
    end
